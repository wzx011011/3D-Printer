...
inline OutputIterator douglas_peucker(
    InputIterator begin, InputIterator end, OutputIterator out, TakeFloaterPredicate take_floater_predicate, PointGetter point_getter)
{
    ...
    if (begin != end) {
        // Supporting in-place reduction and the data type may be generic, thus we are always making a copy of the point value before there
        // is a chance to override input by moving the data to the output.
                    ...
                    if (const SquareLengthType l2 = v.squaredNorm(); l2 == 0) {
                        ...
                        for (auto it = std::next(anchor); it != floater; ++it) {
                            if (SquareLengthType dist_sq = (point_getter(*it) - a).template cast<SquareLengthType>().squaredNorm();
                                !max_dist_sq.has_value() || dist_sq > max_dist_sq) {
                                max_dist_sq = dist_sq;
                                furthest    = it;
                            }
                        }
...
inline OutputIterator douglas_peucker(
    InputIterator begin, InputIterator end, OutputIterator out, const double tolerance, PointGetter point_getter)
{
...
inline double length(const Points& pts) { return length(pts.begin(), pts.end()); }
