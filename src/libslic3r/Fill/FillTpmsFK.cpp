...
Point interpolate(std::vector<std::vector<MarchingSquares2::Point>>& posxy,
                  std::vector<int>                                  p1ij,
                  std::vector<int>                                  p2ij,
                  double                                            v1,
                  double                                            v2,
...
void process_block(int                                               i,
                   ...
                   double                                            contourValue,
                   std::vector<std::vector<MarchingSquares2::Point>>& posxy,
                 ...
                 const FillParams&                                 params)
{
    ...
    vector<vector<MarchingSquares2::Point>> contourPointss(total_size);
                                      ...
                                      Polylines&                     polylines_out)
{
    ...
    for (float y = -(ylen) / 2.0f - 2; y < (ylen) / 2.0f + 2; y = y + delta, i++) {
        j = 0;
        std::vector<MarchingSquares2::Point> colposxy;
        for (float x = -(xlen) / 2.0f - 2; x < (xlen) / 2.0f + 2; x = x + delta, j++) {
            MarchingSquares2::Point pt;
            pt.x = cenpos.x() + x;
            pt.y = cenpos.y() + y;
    ...
    MarchingSquares2::drawContour(contour_value, width, height, data, posxy, polylines, params);
