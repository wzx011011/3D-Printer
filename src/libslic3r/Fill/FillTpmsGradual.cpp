...
static Polylines make_gyroid_waves(double gridZ, double density_adjusted, double line_spacing, double width, double height)
{
    ...
    const double tolerance = std::min(line_spacing / 2, TpmsGradual::PatternTolerance) / unscale<double>(scaleFactor);
...
constexpr double TpmsGradual::PatternTolerance;



void TpmsGradual::_fill_surface_single_brige(
...
void TpmsGradual::cal_scalar_field(const FillParams& params,
                                 std::vector<std::vector<MarchingSquares::Point>>& posxy,
                                 std::vector<std::vector<double>>& data)
{
 ...
 void TpmsGradual::_fill_surface_single(const FillParams&              params,
                                     unsigned int                   thickness_layers,
                                     const std::pair<float, Point>& direction,
                                     ExPolygon                      expolygon,
                                     Polylines&                     polylines_out)
