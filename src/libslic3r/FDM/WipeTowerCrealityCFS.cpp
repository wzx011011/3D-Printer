#include "WipeTowerCrealityCFS.hpp"
...
class WipeTowerWriterCrealityCFS
{
public:
	WipeTowerWriterCrealityCFS(float layer_height, float line_width, GCodeFlavor flavor, const std::vector<WipeTowerCrealityCFS::FilamentParameters>& filament_parameters) :
    ...
    WipeTowerWriterCrealityCFS& change_analyzer_line_width(float line_width) {
        // adds tag for analyzer:
...
#if ENABLE_GCODE_VIEWER_DATA_CHECKING
    WipeTowerWriterCrealityCFS& change_analyzer_mm3_per_mm(float len, float e) {
	...
	WipeTowerWriterCrealityCFS& 			 set_initial_position(const Vec2f &pos, float width = 0.f, float depth = 0.f, float internal_angle = 0.f) {
    ...
    WipeTowerWriterCrealityCFS& 			 set_position(const Vec2f &pos) { m_current_pos = pos; return *this; }

    WipeTowerWriterCrealityCFS&				 set_initial_tool(size_t tool) { m_current_tool = tool; return *this; }

	WipeTowerWriterCrealityCFS&				 set_z(float z) 
		{ m_current_z = z; return *this; }

	WipeTowerWriterCrealityCFS& 			 set_extrusion_flow(float flow)
		{ m_extrusion_flow = flow; return *this; }

	WipeTowerWriterCrealityCFS&				 set_y_shift(float shift) {
        m_current_pos.y() -= shift-m_y_shift;
        m_y_shift = shift;
        return (*this);
    }

    WipeTowerWriterCrealityCFS&            disable_linear_advance() {
...
#if ENABLE_GCODE_VIEWER_DATA_CHECKING
    WipeTowerWriterCrealityCFS& suppress_preview() { change_analyzer_line_width(0.f); m_preview_suppressed = true; return *this; }
    WipeTowerWriterCrealityCFS& resume_preview() { change_analyzer_line_width(m_default_analyzer_line_width); m_preview_suppressed = false; return *this; }
#else
    WipeTowerWriterCrealityCFS& 			 suppress_preview() { m_preview_suppressed = true; return *this; }
	WipeTowerWriterCrealityCFS& 			 resume_preview()   { m_preview_suppressed = false; return *this; }
#endif // ENABLE_GCODE_VIEWER_DATA_CHECKING

	WipeTowerWriterCrealityCFS& 			 feedrate(float f)
	{
    ...
    WipeTowerWriterCrealityCFS& extrude_explicit(float x, float y, float e, float f = 0.f, bool record_length = false, bool limit_volumetric_flow = true)
    {
    ...
    WipeTowerWriterCrealityCFS& extrude_explicit(const Vec2f &dest, float e, float f = 0.f, bool record_length = false, bool limit_volumetric_flow = true)
    { return extrude_explicit(dest.x(), dest.y(), e, f, record_length); }

    // Travel to a new XY position. f=0 means use the current value.
	WipeTowerWriterCrealityCFS& travel(float x, float y, float f = 0.f)
    { return extrude_explicit(x, y, 0.f, f); }

    WipeTowerWriterCrealityCFS& travel(const Vec2f &dest, float f = 0.f)
    { return extrude_explicit(dest.x(), dest.y(), 0.f, f); }

    // Extrude a line from current position to x, y with the extrusion amount given by m_extrusion_flow.
	WipeTowerWriterCrealityCFS& extrude(float x, float y, float f = 0.f)
	{
		float dx = x - m_current_pos.x();
		float dy = y - m_current_pos.y();
    ...
    WipeTowerWriterCrealityCFS& extrude(const Vec2f &dest, const float f = 0.f)
    { return extrude(dest.x(), dest.y(), f); }

    WipeTowerWriterCrealityCFS& rectangle(const Vec2f& ld,float width,float height,const float f = 0.f)
    {
        Vec2f corners[4];
        corners[0] = ld;
    ...
    WipeTowerWriterCrealityCFS& rectangle(const WipeTower::box_coordinates& box, const float f = 0.f)
    {
	...
	WipeTowerWriterCrealityCFS& load(float e, float f = 0.f)
	{
	...
	WipeTowerWriterCrealityCFS& retract(float e, float f = 0.f)
		{ return load(-e, f); }

	// Elevate the extruder head above the current print_z position.
    WipeTowerWriterCrealityCFS& z_hop(float hop, float f = 0.f, std::string _str = "G1")
	{ 
    ...
    // Elevate the extruder head above the current print_z position.
    WipeTowerWriterCrealityCFS& relative_zhop(float hop, float f = 0.f, std::string _str = "G1")
    {
	...
	WipeTowerWriterCrealityCFS& z_hop_reset(float f = 0.f) 
		{ return z_hop(0, f); }

	// Move to x1, +y_increment,
	// extrude quickly amount e to x2 with feed f.
	WipeTowerWriterCrealityCFS& ram(float x1, float x2, float dy, float e0, float e, float f)
	{
	...
	WipeTowerWriterCrealityCFS& cool(float x1, float x2, float e1, float e2, float f)
	{
    ...
    WipeTowerWriterCrealityCFS& set_tool(size_t tool)
	{
		m_current_tool = tool;
		return *this;
	}
	...
	WipeTowerWriterCrealityCFS& set_extruder_temp(int temperature, bool wait = false)
	{
    ...
    // Set extruder temperature, don't wait by default.
    WipeTowerWriterCrealityCFS& custom_set_extruder_temp(int temperature, bool wait = false)
    {
	...
	WipeTowerWriterCrealityCFS& wait(float time)
	{
        if (time==0.f)
            return *this;
	...
	WipeTowerWriterCrealityCFS& speed_override(int speed)
	{
	...
	WipeTowerWriterCrealityCFS& speed_override_backup()
    {
	...
	WipeTowerWriterCrealityCFS& speed_override_restore()
	{
        if (m_gcode_flavor == gcfMarlinLegacy || m_gcode_flavor == gcfMarlinFirmware)
            m_gcode += "M220 R\n";
		return *this;
	...
	WipeTowerWriterCrealityCFS& flush_planner_queue()
	{ 
		m_gcode += "G4 S0\n"; 
		return *this;
	}
	...
	WipeTowerWriterCrealityCFS& reset_extruder()
	{ 
		m_gcode += "G92 E0\n";
		return *this;
	}

	WipeTowerWriterCrealityCFS& comment_with_value(const char *comment, int value)
    {
    ...
    WipeTowerWriterCrealityCFS& set_fan(unsigned speed)
	{
	...
	WipeTowerWriterCrealityCFS& append(const std::string& text) { m_gcode += text; return *this; }
    ...
    WipeTowerWriterCrealityCFS& add_wipe_point(const Vec2f& pt)
    {
        m_wipe_path.push_back(rotate(pt));
        return *this;
    }

    WipeTowerWriterCrealityCFS& add_wipe_point(float x, float y)
    {
...
private:
    ...
    const std::vector<WipeTowerCrealityCFS::FilamentParameters>& m_filpar;
	...
	WipeTowerWriterCrealityCFS& operator=(const WipeTowerWriterCrealityCFS &rhs);
...
WipeTower::ToolChangeResult WipeTowerCrealityCFS::construct_tcr(
    WipeTowerWriterCrealityCFS& writer, bool priming, size_t old_tool, bool is_finish, float purge_volume) const
{
...
WipeTowerCrealityCFS::WipeTowerCrealityCFS(const PrintConfig& config, const PrintRegionConfig& default_region_config,int plate_idx, Vec3d plate_origin, const std::vector<std::vector<float>>& wiping_matrix, size_t initial_tool) :
...
void WipeTowerCrealityCFS::set_extruder(size_t idx, const PrintConfig& config)
{
...
WipeTower::ToolChangeResult WipeTowerCrealityCFS::tool_change(size_t tool, bool extrude_perimeter, bool first_toolchange_to_nonsoluble)
{
	...
	WipeTowerWriterCrealityCFS writer(m_layer_height, m_perimeter_width, m_gcode_flavor, m_filpar);
...
// Ram the hot material out of the melt zone, retract the filament into the cooling tubes and let it cool.
void WipeTowerCrealityCFS::toolchange_Unload(
	WipeTowerWriterCrealityCFS &writer,
...
void WipeTowerCrealityCFS::toolchange_Change(
	WipeTowerWriterCrealityCFS &writer,
...
void WipeTowerCrealityCFS::toolchange_Wipe(
	WipeTowerWriterCrealityCFS &writer,
	const WipeTower::box_coordinates  &cleaning_box,
	float wipe_volume)
{
...
// BBS
WipeTower::box_coordinates WipeTowerCrealityCFS::align_perimeter(const WipeTower::box_coordinates& perimeter_box)
{
...
WipeTower::ToolChangeResult WipeTowerCrealityCFS::finish_layer(bool extrude_perimeter, bool extruder_fill)
{
...
void WipeTowerCrealityCFS::plan_toolchange(
...
void WipeTowerCrealityCFS::plan_tower()
{
    // BBS
...
int WipeTowerCrealityCFS::first_toolchange_to_nonsoluble(
...
WipeTower::ToolChangeResult WipeTowerCrealityCFS::only_generate_out_wall()
{
    size_t old_tool = m_current_tool;
...
void WipeTowerCrealityCFS::generate(std::vector<std::vector<WipeTower::ToolChangeResult>> &result)
{
	if (m_plan.empty())
        return;
	...
	for (const WipeTowerCrealityCFS::WipeTowerInfo& layer : m_plan)
	{
...
std::vector<std::pair<float, float>> WipeTowerCrealityCFS::get_z_and_depth_pairs() const
{
