...
void SmoothCalculator::layer_speed_filter(const int object_id, const int node_id)
{
    ...
    for (int layer_id = layer_start; layer_id <= layer_end; ++layer_id) {
        if (layers_wall_collection[layer_id].empty())
            continue;
        ...
        if (filter_res < node.filter_feedrate)
            node.filter_feedrate = filter_res;
    }
}
...
void SmoothCalculator::filter_layer_time()
{
    ...
    for (int layer_id = 1; layer_id < layers_cooling_time.size(); ++layer_id) {
        if (layers_cooling_time[layer_id] > layer_time_smoothing_threshold)
            continue;
        ...
        for (int filter_pos_idx = 0; filter_pos_idx < guassian_filter.size(); ++filter_pos_idx) {
            ...
            double data_temp = layers_cooling_time[remap_data_pos] > layer_time_smoothing_threshold ? layer_time_smoothing_threshold :
                                                                                                      layers_cooling_time[remap_data_pos];
...
bool SmoothCalculator::layer_time_filter_continue()
{
    for (int layer_id = 1; layer_id < layers_cooling_time.size() - 1; ++layer_id) {
        double layer_time     = layers_cooling_time[layer_id] > layer_time_smoothing_threshold ? layer_time_smoothing_threshold :
                                                                                                 layers_cooling_time[layer_id];
        double layer_time_cmp = layers_cooling_time[layer_id + 1] > layer_time_smoothing_threshold ? layer_time_smoothing_threshold :
                                                                                                     layers_cooling_time[layer_id + 1];
