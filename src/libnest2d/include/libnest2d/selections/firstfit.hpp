        ...
        for (auto it = store_.begin(); it != store_.end() && !cancelled(); ++it) {
            ...
            while(!was_packed && !cancelled()) {
                ...
                if (was_packed && it->get().has_tried_with_excluded) {
                    ...
                    if (fixed_bins.size() >= j/*placers.size()*/)
                        placers[j].preload(fixed_bins[j/*placers.size() - 1*/]);
                }
                ...
                if (placer_not_packed) {
                    ...
                    //} else {
                    //    placers[j - 1].clearItems([](const Item &itm) { return itm.isFixed() && !itm.is_wipe_tower; });
                    //    if (fixed_bins.size() > (j - 1))
                    //        placers[j - 1].preload(fixed_bins[j - 1] /*fixed_bins[placers.size() - 1]*/);
                    //}
                    if (this->unfitindicator_)
                        this->unfitindicator_(it->get().name + " failed on empty bin, forcing new bin creation.");
                }
                ...
                if(!was_packed){
                    ...
                    if (placers.size() >= MAX_NUM_PLATES) {
                        it->get().binId(BIN_ID_UNFIT);
                        if (this->unfitindicator_) {this->unfitindicator_(it->get().name + " failed. Reached MAX_NUM_PLATES (" + std::to_string(MAX_NUM_PLATES) + ")"); }
                        //placer_not_packed == true,but exceed MAX_NUM_PLATES, binId = -1
                        break;
                    }
                    ...
                    size_t new_bin_index = placers.size() - 1;
                    placers.back().plateID(new_bin_index);
                    placers.back().configure(pconfig);
                    if (fixed_bins.size() >= placers.size())
                        placers.back().preload(fixed_bins[new_bin_index]);
