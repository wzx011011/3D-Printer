...
template<class RawShape> struct NfpPConfig
{
    ...
    Coord binItemGap = 0;
    Coord minItemGap = 0; // is equal min_obj_distance
    ...
    NfpPConfig() : rotations({0.0, Pi / 2.0, Pi, 3 * Pi / 2}), alignment(Alignment::CENTER), starting_point(Alignment::CENTER) {}
};
...
template<class RawShape> class EdgeCache
{
    ...
    static double length(const Edge& e) { return std::sqrt(e.template sqlength<double>()); }
    ...
    size_t stride(const size_t N) const
    {
        ...
        return static_cast<Coord>(round(N / pow(N, pow(accuracy_, 1.0 / 3.0))));
    }

    void fetchCorners() const
    {
        if (!contour_.corners.empty())
            return;
        ...
        for (size_t i = 0; i < N_1; i += S) {
            contour_.corners.emplace_back(contour_.distances.at(i) / contour_.full_distance);
        }
    }
    ...
    void fetchHoleCorners(unsigned hidx) const
    {
        ...
        for (size_t i = 0; i < N_1; i += S) {
            hc.corners.emplace_back(hc.distances.at(i) / hc.full_distance);
        }
    }
    ...
    inline Vertex coords(const ContourCache& cache, double distance) const
    {
        ...
        if (cache.distances.empty() || cache.emap.empty())
            return Vertex{};
        if (distance > 1.0)
            distance = std::fmod(distance, 1.0);
        ...
        ret += Vertex(static_cast<Coord>(std::round(ed * std::cos(angle))), static_cast<Coord>(std::round(ed * std::sin(angle))));
...
public:
    ...
    inline EdgeCache(const _Item<RawShape>& item) { createCache(item.transformedShape()); }
     ...
     * circumference.
     */
    inline Vertex coords(double distance) const { return coords(contour_, distance); }
    ...
    inline double circumference() const BP2D_NOEXCEPT { return contour_.full_distance; }

    inline double circumference(unsigned hidx) const BP2D_NOEXCEPT { return holes_[hidx].full_distance; }
    ...
    inline const std::vector<double>& corners(unsigned holeidx) const BP2D_NOEXCEPT
    {
...
template<nfp::NfpLevel lvl> struct Lvl
{
    static const nfp::NfpLevel value = lvl;
};

template<class RawShape>
inline void correctNfpPosition(nfp::NfpResult<RawShape>& nfp, const _Item<RawShape>& stationary, const _Item<RawShape>& orbiter)
{
...
template<class RawShape>
inline void correctNfpPosition(nfp::NfpResult<RawShape>& nfp, const RawShape& stationary, const _Item<RawShape>& orbiter)
{
...
template<class RawShape, class Circle = _Circle<TPoint<RawShape>>> Circle minimizeCircle(const RawShape& sh)
{
    ...
    auto result = solver.optimize_min(
        ...
        opt::initvals(0.0, 0.0), opt::bound(-1.0, 1.0), opt::bound(-1.0, 1.0));
...
template<class RawShape> _Circle<TPoint<RawShape>> boundingCircle(const RawShape& sh) { return minimizeCircle(sh); }

template<class RawShape, class TBin = _Box<TPoint<RawShape>>>
class _NofitPolyPlacer : public PlacerBoilerplate<_NofitPolyPlacer<RawShape, TBin>, RawShape, TBin, NfpPConfig<RawShape>>
{
...
public:
    inline explicit _NofitPolyPlacer(const BinType& bin) : Base(bin), norm_(std::sqrt(abs(sl::area(bin))))
    {
    ...
    template<class Range = ConstItemRange<typename Base::DefaultIter>> PackResult trypack(Item& item, const Range& remaining = Range())
    {
...
private:
    ...
    Shapes calcnfp(const Item& trsh, const Box& bed, Lvl<nfp::NfpLevel::CONVEX_ONLY>)
    {
        ...
        __parallel::enumerate(items_.begin(), items_.end(), [&nfps, &trsh](const Item& sh, size_t n) {
    ...
    template<class Level> Shapes calcnfp(const Item& trsh, Level)
    ...
    struct Optimum
    {
        ...
        unsigned nfpidx;
        int      hidx;
        Optimum(double pos, unsigned nidx) : relpos(pos), nfpidx(nidx), hidx(-1) {}
        Optimum(double pos, unsigned nidx, int holeidx) : relpos(pos), nfpidx(nidx), hidx(holeidx) {}
    };
    ...
    template<class Range = ConstItemRange<typename Base::DefaultIter>> PackResult _trypack(Item& item, const Range& remaining = Range())
    {
        PackResult ret;
        ...
        if (config_.object_function)
            _objfunc = [this](const Item& item) { return config_.object_function(item, this->items_); };
        else {
        ...
        bool pack_object = std::all_of(items_.begin(), items_.end(),
                                       [&](const Item& rawShape) { return rawShape.is_virt_object && !rawShape.is_wipe_tower; });
        ...
        auto overlapWithVirtObject = [&]() -> double {
            ...
            for (const RawShape& nfp : nfps) {
                if (sl::isInside(v, nfp) || sl::touches(v, nfp)) {
                    return 0;
                }
            }
            return 1;
        ...
        if (can_pack == false) {
            ...
            for (Item& mitem : items_) {
                if ((config_.alignment == Config::Alignment::DONT_ALIGN) && !mitem.isInside(bin_))
                    continue;
            ...
            for (auto rot : config_.rotations) {
                ...
                auto rawobjfunc = [_objfunc, iv, startpos](Vertex v, Item& itm) {
                    auto d = v - iv;
                    d += startpos;
                    itm.translation(d);
                    return _objfunc(itm);
                ...
                auto getNfpPoint = [&ecache](const Optimum& opt) {
                    return opt.hidx < 0 ? ecache[opt.nfpidx].coords(opt.relpos) : ecache[opt.nfpidx].coords(opt.hidx, opt.relpos);
                };
                ...
                auto boundaryCheck = [this, alignment, &pile, &nfps, &merged_pile, &getNfpPoint, &item, &bin, &iv,
                                      &startpos](const Optimum& o) {
                ...
                for (unsigned ch = 0; ch < ecache.size(); ch++) {
                    ...
                    __parallel::enumerate(
                        cache.corners().begin(), cache.corners().end(),
                        [&results, &item, &rofn, &nfpoint, ch, accuracy](double pos, size_t n) {
                            Optimizer solver(accuracy);

                            Item itemcpy     = item;
                            auto contour_ofn = [&rofn, &nfpoint, ch, &itemcpy](double relpos) {
                                Optimum op(relpos, ch);
                                return rofn(nfpoint(op), itemcpy);
                            };

                            try {
                                results[n] = solver.optimize_min(contour_ofn, opt::initvals<double>(pos), opt::bound<double>(0, 1.0));
                    ...
                    auto resultcomp = [](const OptResult& r1, const OptResult& r2) { return r1.score < r2.score; };
    ...
    {
        if (items_.empty() || config_.alignment == Config::Alignment::DONT_ALIGN)
            return;
    ...
    inline void finalAlign(Box bbin)
    {
        ...
        { // find a best position inside NFP of fixed items (excluded regions), so the center of pile is cloest to bed center
            ...
            bool   ref_aligned_is_ok    = std::any_of(nfps.begin(), nfps.end(),
                                                      [&ref_aligned](auto& nfp) { return sl::isInside(ref_aligned, nfp); });
