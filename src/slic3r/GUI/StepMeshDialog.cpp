...
#include "Widgets/ColorSlider.hpp"
...
#define FONT_COLOR              wxColour("#010203")
                ...
                wxDEFAULT_DIALOG_STYLE /* | wxRESIZE_BORDER*/), m_file(file)
{
    ...
    std::string icon_path = (boost::format("%1%/images/CrealityPrint-mac_256px.ico")
    ...
    linear_sizer->Add(linear_title, 0, wxALIGN_CENTER_VERTICAL);
    linear_sizer->AddStretchSpacer(1);

    ColorSlider* linear_slider = new ColorSlider(this, wxID_ANY,
                                           SLIDER_SCALE(get_linear_defletion()),
                                           1, 100, wxDefaultPosition,
                                           wxSize(SLIDER_WIDTH, SLIDER_HEIGHT),
                                           wxSL_HORIZONTAL);
    linear_sizer->Add(linear_slider, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, FromDIP(5));
    ...
    linear_input->Bind(wxEVT_KILL_FOCUS, ([this, linear_input](wxFocusEvent& e) {
        ...
        if (validate_number_range(value, 0.001, 0.1)) {
            ...
            CallAfter([linear_input]() {
                linear_input->SetFocus();
                linear_input->GetTextCtrl()->SelectAll();
            });
            m_showingErrorDialog = false;
        }
        e.Skip();
    }));
    ...
    angle_sizer->Add(angle_title, 0, wxALIGN_CENTER_VERTICAL);
    angle_sizer->AddStretchSpacer(1);

    ColorSlider* angle_slider = new ColorSlider(this, wxID_ANY,
                                           SLIDER_SCALE_10(get_angle_defletion()),
                                           1, 100, wxDefaultPosition,
                                           wxSize(SLIDER_WIDTH, SLIDER_HEIGHT),
                                           wxSL_HORIZONTAL);
    angle_sizer->Add(angle_slider, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, FromDIP(5));
    ...
    angle_sizer->Add(angle_input, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, FromDIP(5));
    angle_input->Bind(wxEVT_KILL_FOCUS, ([this, angle_input](wxFocusEvent& e) {
        ...
        if (validate_number_range(value, 0.01, 1)) {
            ...
            SetFocusIgnoringChildren();
            CallAfter([angle_input]() {
                angle_input->SetFocus();
                angle_input->GetTextCtrl()->SelectAll();
            });
            m_showingErrorDialog = false;
        }
        e.Skip();
...
void StepMeshDialog::update_mesh_number_text()
{
    ...
    if (m_button_ok) {
        m_button_ok->SetBackgroundColor(StateColor(std::pair<wxColour, int>(wxColour(180, 180, 180), StateColor::Normal)));
...
void StepMeshDialog::OnMeshNumberUpdated(wxCommandEvent& event)
{
    wxString number_text = event.GetString();
    mesh_face_number_text->SetLabel(number_text);
